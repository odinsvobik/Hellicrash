...
_AIMoney = missionNamespace getVariable "Tom_AIMoney";

_AImodel_1 = missionNamespace getVariable "Tom_AImodel1";
_AImodel_2 = missionNamespace getVariable "Tom_AImodel2";
_AImodel_3 = missionNamespace getVariable "Tom_AImodel3";
_AImodel_4 = missionNamespace getVariable "Tom_AImodel4";
_AImodel_5 = missionNamespace getVariable "Tom_AImodel5";
_AImodel_6 = missionNamespace getVariable "Tom_AImodel6";
_AImodel_7 = missionNamespace getVariable "Tom_AImodel7";
_AImodel_8 = missionNamespace getVariable "Tom_AImodel8";

_amountOfWeapons = missionNamespace getVariable "Tom_amountOfWeapons";
_amountOfItems = missionNamespace getVariable "Tom_amountOfItems";
 

 
_lootWeapons = missionNamespace getVariable "Tom_lootWeapons"; 
_lootItems = missionNamespace getVariable "Tom_lootItems"; 
...
_mapcenter = [5725,7145,0];						//Edit this coordinate it should be roughly the center of the map you are playing on (this one is for Altis)
_centerrad = 1000;									//Radius around the _CenterOfMap (make sure its covering the whole map - Integer)
...
_aiunits = 			[_AImodel_1,_AImodel_2,_AImodel_3,_AImodel_4,_AImodel_5,_AImodel_6,_AImodel_7,_AImodel_8];
...
if (_mode == 2) exitwith {
	...
	//Spawn heli & cargo
	_patrolHeli = missionNamespace getVariable "Tom_Heli";
	_chosenTarget = _patrolHeli call BIS_fnc_selectRandom;
	
	_veh = [[5725,7145,0], 180,_chosenTarget,EAST] call bis_fnc_spawnvehicle;
	_cargo = "B_CargoNet_01_ammo_F" createVehicle [5725,7145,0];
	...
	if (_ai) then {
	_hcunits = [position _cargo, EAST, _aiunits,[],[],[],[],[],360] call BIS_fnc_spawnGroup;
		...
		[_hcg, 0] setWaypointBehaviour "AWARE";
		{
			...
			_rankBot = selectRandom Tom_aiRanks;//POLISHED
			_x setUnitRank _rankBot;//POLISHED
			_skillLevel = selectRandom Tom_aiSkill;//POLISHED
			_x setSkill _skillLevel;//POLISHED
			_uniform = selectRandom Tom_aiUniform;//POLISHED
			_x forceAddUniform _uniform;//POLISHED
			_backpack = selectRandom Tom_aiBackpack;//POLISHED
			_x addBackpack _backpack;//POLISHED
			_vst = selectRandom Tom_aiVest;//POLISHED
			_x addVest _vst;//POLISHED
			_headgear = selectRandom Tom_aiHeadgear;//POLISHED
			_x addHeadgear _headgear;//POLISHED
			
			_counterItemAI = (Tom_aiItemCount select 0) + round random ((Tom_aiItemCount select 1) - (Tom_aiItemCount select 0));//POLISHED
			...
			{//POLISHED
				...
				switch (_randomItem) do 
				{
					...
					case 7:
					{ 
						_items = Tom_aiMags call BIS_fnc_selectRandom;
						_x addMagazineCargoGlobal [_items,1];
					};
					case 8;
					case 9: 
					{ 
						_items = Tom_aiItems call BIS_fnc_selectRandom;
			...
			_aiMoney = floor (random Tom_AIMoney);
			_x setVariable ["ExileMoney",_aiMoney,true];
			_x addEventHandler ["killed", "[_this select 0, _this select 1] execVM 'Custom\Tom\Tom_respect_killedBots.sqf'"];
	...
	_cheli = createVehicle ["Land_Mi8_wreck_F",_crashpos,[], 0, "can_collide"];
